...
void Cache::HandleRequest(uint64_t addr, int bytes, int read, char *content, int &hit, int &time) {
	...
	if (read == 1) {
		...
		if (ReplaceDecision(tag_value, set_value, block_value, read)) {
			...
			stats_.miss_counter++;
		  	/* BYPASS */
			if(config_.bypass && SetIsFull(set_value) && CapacityMiss(tag_value, set_value)){
		...
		if (config_.prefetch) {
			int lower_hit, lower_time;
		  //PrefetchAlgorithm(addr, bytes, read, content, lower_hit, lower_time);
		}
		/* NO PREFETCH */
		else {
...
void Cache::ReplaceAlgorithm(uint64_t tag_value, uint64_t set_value, uint64_t block_value,int &write_back_dirty,
	...
	// if this set is full, use specefic replace algorithm
	// LRU
